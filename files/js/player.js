...
function play(vid, id) {
	...
	if (listbox == 'ul_btn') {
		...
		for (i; i < vRange && i < vlist.length; i++) {
			...
			var vid = vlist[i]['vid'];
			var item = '<li><a href = "javascript:play(vidstr,idstr)"><span>titlevar</span></a></li>'.replace(/titlevar/g, vtitle).replace(/vidstr/g, "'" + vid + "'").replace(/idstr/g, "'" + i + "'");
			if (i == id) {
		...
		for (i; i < vRange && i < vlist.length; i++) {
			var vtitle = vlist[i]['title'];
			var vid = vlist[i]['vid'];
			var item = '<li><a href = "javascript:play(vidstr,idstr)"><img src="http://puui.qpic.cn/qqvideo_ori/0/n0020yrnly7_496_280/0" alt="titlevar"><p>titlevar</p></a></li>'.replace(/titlevar/g, vtitle).replace(/n0020yrnly7_496_280/g, vid + '_496_280').replace(/vidstr/g, "'" + vid + "'").replace(/idstr/g, "'" + i + "'");
			if (i == id) {
				item = item.replace('<li>', '<li class="active">');
			}
			vids.push(item);
